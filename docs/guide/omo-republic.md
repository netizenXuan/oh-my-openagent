...
3. `api-seat` can call `republic_wait` on the original message ID to block within the same session while `docs-seat` reads `republic_inbox`, replies with `answer`, and references the original message ID.
